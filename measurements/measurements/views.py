...
def check_place(data):
    r = requests.get(settings.PATH_PL, headers={"Accept":"application/json"})
    places = r.json()
    for place in places:
...
def MeasurementCreate(request):
    if request.method == 'POST':
        ...
        if check_variable(data_json) == True and check_place(data_json)==True:
            ...
            return HttpResponse("unsuccessfully created measurement. Variable does not exist or place doesnt exist")
...
def MeasurementsCreate(request):
    if request.method == 'POST':
        ...
        for measurement in data_json:
                    if check_variable(measurement) == True or check_place(measurement) == True:
